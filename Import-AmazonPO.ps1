...
if ($needed.Count) {
    ...
    if (-not $tpl.Count) {
        throw ("No purchase order found to copy the supplier settings from " +
               "($(if ($TemplateOrder) { "name = $TemplateOrder" } else { "partner_id = $partnerId" })). " +
               "Create one by hand in OpenProd first, or pass -TemplateOrder with the reference of an existing one. " +
