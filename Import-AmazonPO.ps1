...
if ($needed.Count) {
    ...
    if ($TemplateOrder) { $domain = @(, @('name', '=', $TemplateOrder)) }
    $tpl = Search-OpenProd 'purchase.order' $domain (@('id', 'name') + $needed) 1 'id desc'
    if (-not $tpl.Count) {
        # Distinguish "this supplier has no order yet" from "the search is not
        # returning anything at all", which would be a different problem.
        Write-Host ''
        Write-Host 'No template found. What the search does see:' -ForegroundColor Yellow
        $any = Search-OpenProd 'purchase.order' @() @('id', 'name', 'partner_id') 5 'id desc'
        if (-not $any.Count) {
            Write-Host '  no purchase order at all is visible with this account.' -ForegroundColor Yellow
        }
        else {
            foreach ($a in $any) {
                $pn = $a.partner_id
    ...
    $copied = @()
    foreach ($f in $needed) {
        $v = ConvertTo-Writable $tpl[0].$f
        if ($null -ne $v -and $v -ne '') { $orderDefaults[$f] = $v; $copied += $f }
    }
    Write-Host ("Copied {0} supplier field(s) from {1}: {2}" -f $copied.Count, $tpl[0].name, ($copied -join ', '))
