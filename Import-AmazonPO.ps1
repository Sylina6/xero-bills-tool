...
function Invoke-OpenProd([string] $Route, [hashtable] $Params) {
    ...
    try {
        ...
        # PowerShell 5.1 throws on any non-2xx status and discards the body, but
        # OpenProd puts its real message and its traceback exactly there. Read it
        # back off the error stream, otherwise every failure reads "500 Internal
        # Server Error" and says nothing.
        $detail = ''
        ...
        try {
            ...
            if ($j.error) {
                $msg = $j.error.message
                if ($j.error.data) {
                    if ($j.error.data.message) { $msg = "$msg : $($j.error.data.message)" }
                    elseif ($j.error.data.debug) {
                        # The useful part of a Python traceback is its last lines.
                        $tail = ($j.error.data.debug -split "`n" | Where-Object { $_.Trim() } | Select-Object -Last 4) -join ' | '
                        $msg = "$msg : $tail"
                    }
                }
            }
        ...
        if (-not $msg) { $msg = $detail.Substring(0, [Math]::Min(700, $detail.Length)) }
